...
object AutoCorrector {
        ...
        try {
            ...
            for (locale in standardLocales) {
                try {
                    ...
                    if (context != null) {
                        val customCorrections = ro.pastiera.typeq25.SettingsManager.getCustomAutoCorrections(context, locale)
            ...
            if (context != null) {
                try {
                    val prefs = ro.pastiera.typeq25.SettingsManager.getPreferences(context)
                    ...
                    for ((key, value) in allPrefs) {
                        if (key.startsWith("auto_correct_custom_") && value is String) {
                            ...
                            if (languageCode !in standardLocales) {
                                try {
                                    val customCorrections = ro.pastiera.typeq25.SettingsManager.getCustomAutoCorrections(context, languageCode)
                                    if (customCorrections.isNotEmpty()) {
                                        val customJson = correctionsToJson(customCorrections)
    ...
    fun getCorrection(word: String, locale: String? = null, context: Context? = null): String? {
        ...
        if (context != null) {
            val enabledLanguages = ro.pastiera.typeq25.SettingsManager.getAutoCorrectEnabledLanguages(context)
        ...
        if (targetLocale != "en") {
            ...
            if (context == null || ro.pastiera.typeq25.SettingsManager.isAutoCorrectLanguageEnabled(context, "en")) {
                corrections["en"]?.let { enCorrections ->
                    enCorrections[wordLower]?.let { correction ->
    ...
    fun processText(
        ...
        val enabledLanguages = if (context != null) {
            ro.pastiera.typeq25.SettingsManager.getAutoCorrectEnabledLanguages(context)
        } else {
            emptySet<String>()
        }
