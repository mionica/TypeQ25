...
class AltSymManager(
    ...
    fun reloadSymMappings() {
        if (context != null) {
            val customMappings = ro.pastiera.typeq25.SettingsManager.getSymMappings(context)
    ...
    fun reloadSymMappings2() {
        if (context != null) {
            val customMappings = ro.pastiera.typeq25.SettingsManager.getSymMappingsPage2(context)
            if (customMappings.isNotEmpty()) {
                symKeyMap2.clear()
    ...
    fun handleAltCombination(
        ...
        return if (altChar != null) {
            ...
            if (isPasswordField) {
                altChar = ro.pastiera.typeq25.data.layout.LayoutMappingRepository.convertArabicNumeralsToWestern(altChar)
            }
